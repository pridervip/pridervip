package com.bighuobi.api.gate.filter;

import com.bighuobi.api.enums.RedisIndex;
import com.bighuobi.api.gate.utils.ApplicationContextUtils;
...
public class VisitLimitedFilter implements Filter {
    ...
    private static final String KEY = "huobi-api-visitLimited";
    ...
    public String checkVisitTimes(HttpServletRequest request) {
        ...
        Object value = redisUtils.get(RedisIndex.COMMON.getValue(), key);
        if (null == value) {
            redisUtils.set(RedisIndex.COMMON.getValue(), key, now + ",1", 24 * 3600L);
        } else {
            ...
            if (now - lastRecordTime < interval && times > Integer.parseInt(maxLimited)) {
                ...
                if (now - lastRecordTime < interval) {
                    redisUtils.set(RedisIndex.COMMON.getValue(), key, lastRecordTime + "," + (++times), 24 * 3600L);
                } else {
                    redisUtils.set(RedisIndex.COMMON.getValue(), key, now + ",1", 24 * 3600L);
                }
            }
